	...
	case WM_COMMAND:
	{
		...
		case IDC_EDIT_PREFIX:
		{
			if (HIWORD(wParam) == EN_CHANGE)
			{
				...
				DWORD dwIPpredix = atoi(szIPprefix);
				dwIPmask <<= (32 - dwIPpredix);
...
VOID PrintInfo(HWND hwnd)
{
	...
	sprintf(sz_NumerOfIPs, "Количесто IP-адресов:\t\t%u", 1 << (32 - dwIPprefix));
	sprintf(sz_NumerOfHosts, "Количество хостов:\t\t%u", (1 << (32 - dwIPprefix)) - 2);
